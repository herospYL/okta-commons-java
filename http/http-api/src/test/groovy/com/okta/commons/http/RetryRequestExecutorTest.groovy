...
class RetryRequestExecutorTest {
    ...
    void pauseBeforeRetryNegativeDelay() {
        ...
        def httpResponse = stubResponse("mock error", 400)
        expect HttpException, {requestExecutor.pauseBeforeRetry(1, httpResponse, 31L)}
    }
    ...
    void retryDisabledFirstRequestFailed() {
        ...
        when(requestExecutor.delegate.executeRequest(request)).thenThrow(new HttpException("expected test exception", new Throwable(), true))
